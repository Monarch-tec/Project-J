        ...
        <div className="flex items-center space-x-2">
          {activeList.length > 0 && (
            <button
              onClick={() => {
                exportQuizToPdf({
                  title: activeTab === 'bookmarks' ? 'Saved Study Questions' : 'Challenging Mistakes Notebook',
                  subtitle: `${activeList.length} Targeted Questions for Review`,
                  questions: activeList,
                  includeAnswers: true,
                  includeExplanations: true
                });
              }}
              className="flex items-center space-x-1.5 text-xs font-bold text-indigo-900 bg-white hover:bg-slate-100 px-3.5 py-2 rounded-2xl shadow-sm transition-all"
              title="Download targeted PDF"
            >
              <Download className="w-3.5 h-3.5 text-indigo-600" />
              <span>Export {activeTab === 'bookmarks' ? 'Saved' : 'Mistakes'} PDF</span>
            </button>
          )}
